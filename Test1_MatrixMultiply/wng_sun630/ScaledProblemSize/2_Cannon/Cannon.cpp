...
{
	const int Nrow_A = 3000, Ncol_A = 3000;  //3000*1200*2000 ~40s
	const int Nrow_B = Ncol_A, Ncol_B = 3000;
	...
	if (rank == 0)
		std::cout << "numP = " << numP << ", Matrix size N = " << Nrow_A << std::endl;
	...
	for (i = rowA_LB; i <= rowA_UB; i++)
		for (j = colA_LB; j <= colA_UB; j++) {
			...
			matrixA_local[index_local] = (i*Ncol_A + j) % 5;
		}
	...
	for (i = rowB_LB; i <= rowB_UB; i++)
		for (j = colB_LB; j <= colB_UB; j++) {
			...
			matrixB_local[index_local] = (i*Ncol_B + j) % 10;
		}
